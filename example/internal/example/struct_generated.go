...
func StructEncode(dst []byte, s *Struct) []byte {
	...
	// Encode VarInt(intypes.VI).
	dst = binary.AppendVarint(dst, int64(s.VarInt))

	// Encode VarUint(intypes.VU).
...
func StructDecode(s *Struct, src []byte) (err error) {
	...
	// Decode VarInt(intypes.VI).
	{
		val, off := binary.Varint(src)
		if off <= 0 {
			if off == 0 {
				return errors.New("decode s.VarInt(intypes.VI): record buffer is too small")
			}
			return errors.New("decode s.VarInt(intypes.VI): malformed varint sequence")
		}
		s.VarInt = int(val)
		src = src[off:]
	}

	// Decode VarUint(intypes.VU).
	{
		val, off := binary.Uvarint(src)
		if off <= 0 {
			if off == 0 {
				return errors.New("decode s.VarUint(intypes.VU): record buffer is too small")
			}
			return errors.New("decode s.VarUint(intypes.VU): malformed uvarint sequence")
		}
		s.VarUint = uint(val)
		src = src[off:]
	}
